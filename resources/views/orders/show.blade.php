...
@section('content')
    <div class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Hero Header -->
            <div
                class="relative bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-700 rounded-3xl p-8 mb-8 text-white overflow-hidden">
                ...
                <div class="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between">
                    <div class="mb-6 lg:mb-0">
                        <div class="flex items-center mb-3">
                            <div
                                class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center mr-4 backdrop-blur-sm">
                            ...
                            <div>
                                <h1 class="text-3xl lg:text-4xl font-bold">Detail Pesanan</h1>
                                <p class="text-white/80 text-lg">{{ $order->order_number }}</p>
                            </div>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                        @php
                            // Determine display status based on order status and delivery progress
                            if ($order->delivery) {
                                switch ($order->delivery->status) {
                                    case 'assigned':
                                        $displayStatus = 'Ditugaskan ke Kurir';
                                        $displayColor = 'bg-blue-500/90';
                                        $statusIcon = '👤';
                                        break;
                                    case 'picked_up':
                                        $displayStatus = 'Barang Diambil Kurir';
                                        $displayColor = 'bg-yellow-500/90';
                                        $statusIcon = '📦';
                                        break;
                                    case 'in_transit':
                                        $displayStatus = 'Dalam Pengiriman';
                                        $displayColor = 'bg-purple-500/90';
                                        $statusIcon = '🚚';
                                        break;
                                    case 'delivered':
                                        $displayStatus = 'Barang Terkirim';
                                        $displayColor = 'bg-green-500/90';
                                        $statusIcon = '✅';
                                        break;
                                    case 'failed':
                                        $displayStatus = 'Pengiriman Gagal';
                                        $displayColor = 'bg-red-500/90';
                                        $statusIcon = '❌';
                                        break;
                                    default:
                                        $displayStatus = $order->status_label;
                                        $displayColor = 'bg-gray-500/90';
                                        $statusIcon = '📋';
                                }
                            } else {
                                $displayStatus = $order->status_label;
                        ...
                        <div class="flex items-center gap-3">
                            <div
                                class="px-6 py-3 {{ $displayColor }} rounded-2xl backdrop-blur-sm border border-white/20 flex items-center">
                                <span class="mr-2 text-lg">{{ $statusIcon }}</span>
                                <span class="font-semibold">{{ $displayStatus }}</span>
                            </div>
                        </div>
                        ...
                        @if ($order->canBeCancelled())
                            <form action="{{ route('orders.cancel', $order) }}" method="POST" class="inline">
                                ...
                                <button type="submit"
                                    ...
                                    class="px-6 py-3 bg-red-500/90 hover:bg-red-600/90 rounded-2xl backdrop-blur-sm border border-white/20 transition-all transform hover:scale-105 flex items-center font-semibold">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="xl:col-span-3 space-y-8">
                    ...
                    <div class="bg-white rounded-3xl shadow-xl p-8 border border-gray-100">
                        ...
                        <div class="relative">
                            @php
                                $steps = [
                                    'created' => ['label' => 'Pesanan Dibuat', 'icon' => '📝', 'completed' => true],
                                    'paid' => [
                                        'label' => 'Pembayaran',
                                        'icon' => '💳',
                                        'completed' => $order->payment && $order->payment->status === 'paid',
                                    ],
                                    'processing' => [
                                        'label' => 'Diproses',
                                        'icon' => '⚡',
                                    ...
                                    'shipped' => [
                                        ...
                                        'completed' =>
                                            $order->delivery &&
                                            in_array($order->delivery->status, [
                                                'picked_up',
                                                'in_transit',
                                                'delivered',
                                            ]),
                                    ...
                                    'delivered' => [
                                        'label' => 'Selesai',
                                        'icon' => '🎉',
                                        'completed' => $order->delivery && $order->delivery->status === 'delivered',
                                    ],
                                ];
                            @endphp
                            ...
                            <div class="flex items-center justify-between mb-8">
                                @foreach ($steps as $key => $step)
                                    <div class="flex flex-col items-center relative">
                                        <div
                                            class="w-16 h-16 rounded-2xl {{ $step['completed'] ? 'bg-gradient-to-r from-green-400 to-green-600' : 'bg-gray-200' }} flex items-center justify-center text-2xl shadow-lg transform transition-all hover:scale-105">
                                            {{ $step['completed'] ? '✅' : $step['icon'] }}
                                        </div>
                                        <span
                                            class="text-sm font-medium mt-2 {{ $step['completed'] ? 'text-green-600' : 'text-gray-500' }}">
                                            {{ $step['label'] }}
                                        </span>

                                        @if (!$loop->last)
                                            <div
                                                class="absolute top-8 left-16 w-full h-1 {{ $step['completed'] && $steps[array_keys($steps)[$loop->index + 1]]['completed'] ? 'bg-green-400' : 'bg-gray-200' }} z-0">
                                            </div>
                                        @endif
                                    </div>
                                @endforeach
                    ...
                    <div class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
                        <div class="p-8 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-gray-100">
                            <h2 class="text-2xl font-bold text-gray-900 flex items-center">
                                ...
                                Produk Pesanan
                                <span class="ml-3 px-3 py-1 bg-blue-100 text-blue-800 rounded-xl text-sm font-medium">
                            ...
                            @foreach ($order->orderItems as $item)
                                ...
                                <div class="p-8 hover:bg-gray-50 transition-colors">
                                    <div class="flex items-start gap-6">
                                        <!-- Product Image -->
                                        <div
                                            class="w-24 h-24 bg-gray-200 rounded-2xl overflow-hidden flex-shrink-0 shadow-md">
                                            @if ($imageCount > 0)
                                            ...
                                            @if ($existingReview)
                                                <div
                                                    ...
                                                    <div class="flex items-center gap-3 mb-3">
                                                        <span
                                                            class="text-sm font-semibold text-green-800 flex items-center">
                    ...
                    <div class="bg-white rounded-3xl shadow-xl p-8 border border-gray-100">
                        ...
                        @if ($order->notes)
                            <div
                                class="mt-6 p-6 bg-gradient-to-br from-yellow-50 to-orange-50 rounded-2xl border border-yellow-100">
                ...
                <div class="xl:col-span-1 space-y-6">
                    ...
                    <div class="bg-white rounded-3xl shadow-xl p-6 border border-gray-100 sticky top-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <div
                                class="flex justify-between items-center py-3 bg-gradient-to-r from-green-50 to-emerald-50 -mx-2 px-2 rounded-xl">
                            ...
                            <div class="space-y-4">
                                ...
                                @if ($order->payment->reference_number)
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span
                                            class="font-mono text-sm bg-gray-100 px-2 py-1 rounded">{{ $order->payment->reference_number }}</span>
                                    </div>
                                @endif
                            ...
                            @if ($order->payment->proof_image)
                                <div class="mt-6">
                                    ...
                                    <img src="{{ asset('storage/' . $order->payment->proof_image) }}"
                                        ...
                                        class="mt-3 w-full rounded-2xl border border-gray-200 cursor-pointer hover:shadow-lg transition-shadow"
                            ...
                            @if ($order->payment->method === 'transfer' && $order->payment->status === 'pending')
                                <div
                                    ...
                                    @if (!$order->payment->proof_image)
                                        ...
                                        <div class="bg-white p-4 rounded-xl border mb-4 text-sm">
                                            ...
                                            <div class="space-y-1 text-gray-700">
                                                <p>No. Rekening: <span
                                                        class="font-mono bg-gray-100 px-1 rounded">1234567890</span></p>
                                        ...
                                        <form action="{{ route('payments.upload-proof', $order->payment) }}"
                                    ...
                                    @else
                                        ...
                                        <img src="{{ asset('storage/' . $order->payment->proof_image) }}"
                                            alt="Bukti Transfer" class="w-full rounded-xl border cursor-pointer"
                                            onclick="showImageModal(this.src)">
                                    @endif
                                </div>
                            @endif
                            ...
                            @if ($order->payment->method === 'qris' && $order->payment->status === 'pending')
                                <div
                                    ...
                                    <div class="bg-white p-4 rounded-xl border text-center mb-4">
                                        ...
                                        <p class="font-bold text-purple-600 text-lg">Rp
                    ...
                    @if ($order->delivery)
                        <div class="bg-white rounded-3xl shadow-xl p-6 border border-gray-100">
                            <h2 class="text-xl font-bold text-gray-900 mb-6 flex items-center">
                                <div
                                    class="w-6 h-6 bg-gradient-to-r from-orange-500 to-red-600 rounded-lg flex items-center justify-center mr-3">
                                    <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="space-y-4">
                                ...
                                @if ($order->delivery->tracking_number)
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span
                                            class="font-mono text-sm bg-gray-100 px-2 py-1 rounded">{{ $order->delivery->tracking_number }}</span>
                                    </div>
                                @endif
                            </div>

                            <div class="mt-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl">
                                <p class="text-sm text-blue-800 font-medium flex items-center">
                                    <span class="mr-2">📦</span>
                                    {{ $order->delivery->status_label }}
                                </p>
                                {{-- @if ($order->delivery->notes)
                                    <p class="text-xs text-blue-600 mt-2 italic">{{ $order->delivery->notes }}</p>
                                @endif --}}
                            </div>
                        </div>
                    @endif
